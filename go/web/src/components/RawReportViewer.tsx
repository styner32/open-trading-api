...
// Non-global on purpose: /g regexes carry lastIndex state across .test() calls.
const EUC_KR_CHARSET = /charset\s*=\s*["']?\s*euc-kr\s*["']?/i;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Robust regex heuristic for EUC-KR
        if (EUC_KR_CHARSET.test(decoded)) {
             try {
        ...
        decoded = decoded.replace(/(charset\s*=\s*["']?)\s*euc-kr\s*(["']?)/gi, "$1utf-8$2");
