...
with open(config_root + 'kis_devlp.yaml', encoding='UTF-8') as f:
    _cfg = yaml.safe_load(f) # SEC: Use safe_load to prevent insecure deserialization
...
def read_token():
    try:
        ...
        with open(token_tmp, encoding='UTF-8') as f:
            tkg_tmp = yaml.safe_load(f) # SEC: Use safe_load to prevent insecure deserialization
